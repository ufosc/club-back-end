...
fn replace_member(username: &str, modifier: &Member) {
	...
	let update_result = diesel::update(members::table)
		.set(modifier)
		.get_result::<Member>(&connection);
}
...
// Must be differnt from other modify events, because the username is a key value in the database
fn modify_ufl_username(username: &str, string_replace: &str) {
	let connection = database::establish_connection();

	let temp_member = members::table.filter(members::ufl_username.eq(&username));
...
fn modify_info_filled_out(username: &str, bool_replace: &bool) {
	let connection = database::establish_connection();
...
fn modify_acm_shareable(username: &str, bool_replace: &bool) {
	let connection = database::establish_connection();
...
fn modify_in_email_list(username: &str, bool_replace: &bool) {
	let connection = database::establish_connection();
...
mod tests {
	...
	#[test]
	fn modify_member_discord_username() {
		clear_table();
		let connection = database::establish_connection();
	...
	#[test]
	fn modify_member_github_username() {
		clear_table();
		let connection = database::establish_connection();
	...
	#[test]
	fn modify_member_server_username() {
		clear_table();
		let connection = database::establish_connection();
	...
	fn modify_info() {
		...
		modify_info_filled_out(&new_member.ufl_username, &true);
	...
	#[test]
	fn modify_memeber_acm_shareable() {
		clear_table();
		let connection = database::establish_connection();
		...
		modify_acm_shareable(&new_member.ufl_username, &true);
	...
	#[test]
	fn modify_member_in_email_list() {
		clear_table();
		let connection = database::establish_connection();
		...
		modify_in_email_list(&new_member.ufl_username, &true);
